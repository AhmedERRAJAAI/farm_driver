...
class PoussForm extends StatefulWidget {
  const PoussForm({super.key});
  static const routeName = 'pouss-form/';
...
class _PoussFormState extends State<PoussForm> {
  ...
  void fetchLots(site) async {
    try {
      await Provider.of<SitesBatsProvider>(context, listen: false).fetchLots(site).then((_) {
        setState(() {
          bats = Provider.of<SitesBatsProvider>(context, listen: false).lotsData.where((obj) => obj.prod == false).toList();
        });
  ...
  Widget build(BuildContext context) {
    ...
    bats = Provider.of<SitesBatsProvider>(context).lotsData.where((obj) => obj.prod == false).toList();
    return Scaffold(
      appBar: AppBar(
        title: Text(
          "Poussiniére",
          style: TextStyle(color: Colors.orange.shade700, fontStyle: FontStyle.normal, fontSize: 18),
        ),
                ...
                children: [
                  isLoading
                      ...
                      : failedToFetch
                          ...
                          : InitFormSelect(selectName: 'site', themeColor: Colors.orange.shade600, inputsOptions: sites, fetch: fetchLots, selectedVal: batGetter),
                  InitFormSelect(selectName: 'bâtiment', themeColor: Colors.orange.shade600, inputsOptions: bats, fetch: fetchPrefilled, selectedVal: siteGetter),
                  Container(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: Colors.orange.shade700, // Border color
